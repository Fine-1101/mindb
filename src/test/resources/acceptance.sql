-- ============================================================
-- MiniDB 验收测试脚本 acceptance.sql（内存模式主脚本）
-- 对照评分表 40 分逐项覆盖，-- expect: OK | LEXER | PARSER | SEMANTIC
--
-- 运行：mvn compile exec:java -Dexec.mainClass=com.minidb.MiniDB ^
--        -Dexec.args="src/test/resources/acceptance.sql"
--
-- ============ 运行矩阵（评分项 → 覆盖方式） ============
-- 词法分析(4)      本脚本 1.1 节 + 单测 LexerTest/ParserFuzzTest
-- 语法分析(4)      本脚本 1.2 节 + 单测 ParserTest/ParserD5Test
-- 语义分析(4)      本脚本 1.3 节 + 单测 SemanticAnalyzerTest/TypeRulesTest
-- 执行计划生成(4)  本脚本 1.4 节（语句可执行性；S-Plan 层需交互模式 .trace 复核）
--                  + 单测 PlannerTest/OptimizerTest/PlanPrinterTest/OptimizationEquivalenceTest
-- 页式存储管理(4)  本脚本 2.1 节（页分配/释放/读写/跨页）
--                  + 单测 ParameterizedPageTest/DiskPageTest/MemoryPageTest（逐字节对拍/重启恢复）
-- 缓存机制(4)      本脚本 2.2 节（固定交替访问序列）
--                  + 单测 InMemoryBufferPoolTest/BufferLogTest（LRU 命中/淘汰/回写统计）
-- 接口与集成(4)    本脚本 2.3 节（统一接口下双表交替）+ 单测 EngineDiskIntegrationTest
-- 执行引擎(4)      本脚本 3.1 节（SeqScan/Filter/Project/Insert/Delete/Update/Sort/Join/聚合）
-- 存储引擎与目录(4) 本脚本 3.2 节（序列化往返/目录大小写）
--                  + 磁盘两段式脚本（持久化/数据恢复，见 acceptance-disk-*.sql）
--                  + 单测 RowEncoderTest/PersistentCatalogTest
-- 系统集成(4)      本脚本整体 = CLI"SQL输入—编译—执行—存储—结果返回"全链路
--
-- ============ 脚本模式约定 ============
-- 1) 按顶层 ';' 切分逐条执行，单条错误不中断后续语句；
-- 2) L/P/T 组需展示 Tokens/AST/Plan 段的完整五段输出，请在交互模式
--    用 .trace 复核（见 acceptance-tests.md 0.1）；
-- 3) 脚本模式不支持无 FROM 的 SELECT（语法层要求 FROM），
--    L3/L4/L5 以带 FROM 的等价形式验证同一 Token 特性；
-- 4) 按验收文档 0.2 约定"各用例独立使用演示数据"，
--    写操作用例组（S11/T6/E4-E6）之后重置数据。
-- ============================================================

-- ===== 第 0 节：演示数据（acceptance-tests.md 0.2） =====
-- expect: OK
...
-- 数据持久化 / 重启数据恢复：见 acceptance-disk-setup.sql + acceptance-disk-verify.sql
--（--disk 两段式：会话1建表插数据 exit 落盘，会话2重启读回）
